...
module.exports.checkTxValidity = ( txRoleEntities, amount, timeSecondsUNIX, reference, txType ) => {
  ...
  const deactiveRecipient = recipients.filter( obj => {return obj.profile.status != 'active' } );
  ...
  return senders.length === 0 ? i18n.strErTx175 : // 'No valid sender to initiate transfer'
    senders.length > 1 ? 'Only one sender allowed' :
      ...
      // TODO: sending to self >>> txType != i18n.str50020 && recipients.indexOf(senders[0]) != -1 ? errorTx(i18n.strErTx140 + ' <span class="currency-unit">' + i18n.str60010 + '</span> ' + i18n.strErTx141) :
        systemInit.communityGovernance.limitTransactions === true && !senders[0].profile.verified && recipients.find( recipient => {return recipient.profile.verified != false} ) ? i18n.strErTx125 : // 'For security references you can only transact with unverified members...'
          senders[0].profile.status != 'active' ? senders[0].profile.title + ' ' + senders[0].profile.tag + ' ' + i18n.strErTx180 : // '... account deactivated'
            ...
            // TODO: all expiry date rules
              ( txType == i18n.str50020 || txType == i18n.str50030 ) && reference.length < 11 ? i18n.strErTx240 : // 'You must provide a more detailed reference...'
                txType == i18n.str50020 && ['pool', 'contribution'].indexOf( senders[0].profile.role ) == -1 ? i18n.strErTx170 + ' ' + senders[0].profile.title + ' ' + senders[0].profile.tag : // 'You cannot request from ...'
                  txType == i18n.str50020 && amount > requestLimit ? i18n.strErTx160 + ' ' + requestLimit + ' ' + i18n.strErTx163 : // 'Request limit of ... exceeded'
                    txType == i18n.str50030 && ( senders[0].properties.creator != txRoleEntities[2].profile.title || senders[0].properties.creatorTag != txRoleEntities[2].profile.tag ) ? i18n.strErTx250 : // 'You must be the account-creator to transfer'
