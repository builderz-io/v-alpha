  ...
  FusedController
{
  ...
  // blocks mined manually (for local testing)
  ...
  function calcNumCompletedPeriods(
    ...
    uint256 blocksSinceLastGeneration = _blockNumber.sub(_lastGenerationBlock);
  ...
  function calcContribution(
    ...
    uint256 contributionFeeMultiplier = (100 * 10**contributionFeeDecimals)**2;
    return
  ...
  /** @notice Unapprove the specified account so that it no longer receives
        generation */
  function unapproveAccount(address _account) external onlyController fused(6) {
  ...
  function verifyAccount(address _account) external onlyController {
    ...
    emit TransferSummary(address(this), _account, initialBalance, 0, 0, 0, 0);
  }
  ...
  function updateLifetime(uint256 _lifetime) external onlyController fused(5) {
    lifetime = _lifetime;
    ...
    fused(4)
  {
    ...
    revert("Currently not possible to change generation period after deploy.");
    generationPeriod = _generationPeriod;
