...
contract("VICoin", async (accounts) => {
  ...
  it("calculates generation correctly", async () => {
    ...
    const generationAmount = await vicoin.calcGeneration.call(
      20,
      10,
      10,
      9000,
    ...
    assert.equal(
      Number(generationAmount.valueOf()),
      9000,
      "Generation amount not correct"
    );

    // Caps out at one lifetime worth of blocks
    const generationAmount2 = await vicoin.calcGeneration.call(
      2000,
      10,
      10,
      9000,
    ...
    assert.equal(
      Number(generationAmount2.valueOf()),
      9000,
      "Generation amount does not cap out at one lifetime worth of blocks "
    );

    // Unless there are multiple generation periods since the last block, within the lifetime
    const generationAmount3 = await vicoin.calcGeneration.call(
      2000,
      10,
      10,
      9000,
    ...
    assert.equal(
      Number(generationAmount3.valueOf()),
      89991,
      "Generation amount doesn't handle multiple generation periods within the lifetime"
    );

    // Longer lifetime
    const generationAmount4 = await vicoin.calcGeneration.call(
      30,
      10,
      20,
      9000,
    ...
    assert.equal(
      Number(generationAmount4.valueOf()),
      13500,
      "Generation amount doesn't handle longer lifetime correctly"
    );

    // Part way through the second period
    const generationAmount5 = await vicoin.calcGeneration.call(
      25,
      10,
      20,
      9000,
    ...
    assert.equal(
      Number(generationAmount5.valueOf()),
      9000,
      "Generation amount doesn't handle part way through the second period"
    );

    // Part way through the third period
    const generationAmount6 = await vicoin.calcGeneration.call(
      35,
      10,
      20,
      9000,
    ...
    assert.equal(
      Number(generationAmount6.valueOf()),
      13500,
      "Generation amount doesn't handle part way through the third period"
    );

    // Part way through the third period, small decay
    const generationAmount7 = await vicoin.calcGeneration.call(
      35,
      10,
      200,
      9000,
    ...
    assert.equal(
      Number(generationAmount7.valueOf()),
      17550,
      "Generation amount doesn't handle part way through the third period, small decay"
    );

    // Part way through the fourth period, small decay
    const generationAmount8 = await vicoin.calcGeneration.call(
      47,
      10,
      200,
      9000,
    ...
    assert.equal(
      Number(generationAmount8.valueOf()),
      25672,
      "Generation amount doesn't handle part way through the fourth period, small decay"
    );

    // Part way through the fifth period, small decay
    const generationAmount9 = await vicoin.calcGeneration.call(
      51,
      10,
      200,
      9000,
    ...
    assert.equal(
      Number(generationAmount9.valueOf()),
      33388,
      "Generation amount doesn't handle part way through the fifth period, small decay"
    );

    // Part way through the fifth period, small decay
    const generationAmount10 = await vicoin.calcGeneration.call(
      69,
      10,
      200,
      9000,
    ...
    assert.equal(
      Number(generationAmount10.valueOf()),
      40718,
      "Generation amount doesn't handle part way through the sixth period, small decay"
    );
  });
  ...
  it("calculates blocks to zero", async () => {
    ...
    assert.equal(
      Number(
        await vicoin.calcZeroBlock.call(1000, 0, /*460, */ 460, 10, 0).valueOf()
      ),
      470,
      "Calculation not correct"
    );
    assert.equal(
      Number(
        await vicoin.calcZeroBlock
          .call(1000, 0, /*460, */ 460, 10, 470)
          .valueOf()
      ),
      470,
      "Calculation not correct"
    ...
    assert.equal(
      Number(
        await vicoin.calcZeroBlock
          .call(10000, 10000, /*460, */ 469, 10, 470)
          .valueOf()
      ),
      475,
      "Calculation not correct"
  ...
  it("calculates fees correctly", async () => {
    ...
    assert.equal(
      Number(
        await vicoin.calcFeesIncContribution.call(1000000, 3300).valueOf()
      ),
      330000,
      "Calculation not correct"
    );
    assert.equal(
      Number(await vicoin.calcFeesIncContribution.call(10, 3300).valueOf()),
      3,
      "Calculation not correct"
    );
  });
  ...
  it("calculates contribution correctly", async () => {
    const vicoin = await VICoin.deployed();
    assert.equal(
      Number(await vicoin.calcContribution.call(1000000, 3300, 1000).valueOf()),
      33000,
      "Calculation not correct"
    );
    assert.equal(
      Number(await vicoin.calcContribution.call(10, 3300, 1000).valueOf()),
      0,
      "Calculation not correct"
    );
  });
  ...
  it("calculates fees to burn correctly", async () => {
    const vicoin = await VICoin.deployed();
    assert.equal(
      Number(await vicoin.calcFeesToBurn.call(1000000, 3300, 1000).valueOf()),
      330000 - 33000,
      "Calculation not correct"
    );
    assert.equal(
      Number(await vicoin.calcFeesToBurn.call(10, 3300, 1000).valueOf()),
      3,
      "Calculation not correct"
    );
  });
  ...
  it("can mine", async () => {
    ...
    assert.isAtLeast(
      Number(blockNumberAfter.valueOf()),
      Number(blockNumberBefore.valueOf()) + 50,
  ...
  it("can transfer", async () => {
    const vicoin = await VICoin.deployed();
    const blockNumberBefore = await vicoin.getBlockNumber.call();
    const transferred = await vicoin.transfer(accounts[6], 1000, {
      from: accounts[0],
    });
  ...
  it("balance after 6 blocks is 40%", async () => {
    ...
    assert.equal(
      ...
      900,
      "Balance not correct after one block"
    );
    const blockNumberBefore = await vicoin.getBlockNumber.call();
    ...
    assert.equal(
      Number(burnedBalanceAfterMining.valueOf()),
      400,
      "Balance not correct after 2 blocks"
    );
  });
  ...
  it("balance after 8 blocks is 20%", async () => {
    ...
    assert.equal(
      Number(burnedBalanceAfterMining.valueOf()),
      200,
      "Balance not correct after 8 blocks"
    );
  });
  ...
  it("balance after 10 blocks is zero", async () => {
    ...
    assert.equal(
      Number(burnedBalanceAfterMining.valueOf()),
      0,
      "Balance not correct after 10 blocks"
    );
  });
  ...
  it("balance after 15 blocks is zero", async () => {
    ...
    assert.equal(
      Number(burnedBalanceAfterMining.valueOf()),
      0,
      "Balance not correct after 15 blocks"
    );
  });
  ...
  it("large incoming transfer renews lifetime", async () => {
    ...
    assert.equal(
      ...
      Number(thisBlock) + 10,
      "Lifetime not extended correctly"
    );
  });
  ...
  it("Medium incoming transfer renews partial lifetime", async () => {
    ...
    const zeroBlockBefore = await vicoin.zeroBlock.call(accounts[0]).valueOf();
    const liveBalanceBefore = await vicoin.liveBalanceOf
      .call(accounts[0])
      .valueOf();
    await vicoin.transfer(accounts[0], 800, { from: accounts[8] });

    const zeroBlockAfter = await vicoin.zeroBlock.call(accounts[0]).valueOf();
    const liveBalanceAfter = await vicoin.liveBalanceOf
      .call(accounts[0])
      .valueOf();
    ...
    assert.isAtMost(
      ...
      2,
      "Difference was more than expected - expected around 50% of full lifetime to be added"
    );
  });
  ...
  it("can disable contribution change function", async () => {
    ...
    await truffleAssert.reverts(
      vicoin.updateCommunityContribution(2000),
  ...
  it("can blow all fuses", async () => {
    ...
    await truffleAssert.reverts(
      vicoin.updateTransactionFee(4444),
      "Function fuse has been triggered"
    );
  });
});
