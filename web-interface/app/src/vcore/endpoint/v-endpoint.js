const VAction = ( function() { // eslint-disable-line no-unused-vars
  ...
  */
  ...
  function checkForTriggers( caseTxt, triggers ) {
    ...
    const checkParts = caseTxt.trim().split( ' ' );
  ...
  function setEndpoint( which, data ) {
    ...
    if ( which == 'new message' ) {
      ...
      else if ( caseTxt.match( /[a-zA-Z0-9+]/ ) === null ) {
        return Promise.resolve( { status: 'error', message: 'not a valid message' } );
      }

      else if ( triggers.misspellingsEN.concat( triggers.misspellingsDE ).indexOf( caseTxt.substr( 0, 4 ) ) >= 0 ) {
       ...
       */
        ...
        const caseArray = checkForTriggers( caseTxt, triggers );
