  ...
  $.fn.leafletLocationPicker = function( opts, onChangeLocation ) {
    ...
    function parseLocation( loc ) {
      ...
      case 'object':
        ...
        if( loc.hasOwnProperty( 'lng' ) ) {lng = loc.lng}
        else if( loc.hasOwnProperty( 'lon' ) ) {lng = loc.lon}
        else if( loc.hasOwnProperty( 'longitude' ) ) {lng = loc.longitude}
    ...
    function buildMap( self ) {
      ...
      else {self.$map.appendTo( 'body' )}

      if( self.location ) {opts.map.center = self.location}

      if( typeof opts.layer === 'string' && baseLayers[opts.layer] ) {opts.map.layers = L.tileLayer( baseLayers[opts.layer] )}

      else if( opts.layer instanceof L.TileLayer
					|| opts.layer instanceof L.LayerGroup ) {opts.map.layers = opts.layer}

      else {opts.map.layers = L.tileLayer( baseLayers.OSM )}
      ...
      if( opts.locationMarker ) {self.marker = buildMarker( self.location ).addTo( self.map )}
